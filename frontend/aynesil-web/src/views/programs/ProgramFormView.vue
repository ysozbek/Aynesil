...
onMounted(async () => {
  await refData.getValues('PROGRAM_TYPE').then(v => { programTypes.value = v })
  ...
  if (isEdit.value && id.value) {
    ...
    if (program) {
      ...
      form.programTypeId = program.programTypeId ?? ''
      form.description = program.description ?? ''
      form.rowVersion = program.rowVersion
    }
  }
})
...
function validate(): boolean {
  ...
  let valid = true
  if (!form.code.trim()) {
    errors.code = t('validation.required', { field: t('program.code') })
    valid = false
  }
  if (!form.name.trim()) {
    errors.name = t('validation.required', { field: t('program.name') })
    valid = false
  }
  return valid
}
...
async function submit() {
  ...
  try {
    if (isEdit.value && id.value) {
      await store.updateProgram(id.value, {
        code: form.code,
        name: form.name,
      ...
      router.push({ name: 'program-detail', params: { id: id.value } })
    } else {
      ...
      router.push({ name: 'program-detail', params: { id: result.id } })
    }
  } catch (e: unknown) {
...
<template>
  <div class="max-w-3xl mx-auto">
    <PageHeader
      :title="isEdit ? t('program.edit') : t('program.create')"
    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div class="rounded-xl border border-border bg-[--color-card] p-5 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-sm font-medium text-foreground mb-1">{{ t('program.code') }} *</label>
            <input
              v-model="form.code"
              type="text"
        ...
        <div>
          <label class="block text-sm font-medium text-foreground mb-1">{{ t('program.programType') }}</label>
          <select
