    ...
    : IRequestHandler<GetAssessmentSessionsQuery, PaginatedResult<AssessmentSessionListItemDto>>
{
        ...
        GetAssessmentSessionsQuery req, CancellationToken ct)
    {
        ...
        // Filter on the entity first — EF cannot translate Where on projected DTOs after joins.
        var q = _db.AssessmentSessions.AsNoTracking();

        if (req.CorporationId.HasValue)
            q = q.Where(s => s.CorporationId == req.CorporationId);

        if (req.CampusId.HasValue)
            q = q.Where(s => s.CampusId == req.CampusId);

        if (req.TemplateId.HasValue)
            q = q.Where(s => s.TemplateId == req.TemplateId);
        ...
        if (req.AssessorId.HasValue)
            q = q.Where(s => s.AssessorId == req.AssessorId);

        if (req.From.HasValue)
            q = q.Where(s => s.ScheduledAt >= req.From);

        if (req.To.HasValue)
            q = q.Where(s => s.ScheduledAt <= req.To);

        q = req.SortBy?.ToLowerInvariant() switch
        {
            "scheduledat" => req.IsDescending
                ? q.OrderByDescending(s => s.ScheduledAt)
                : q.OrderBy(s => s.ScheduledAt),
            "status" => req.IsDescending
                ? q.OrderByDescending(s => s.Status)
                : q.OrderBy(s => s.Status),
            _ => q.OrderByDescending(s => s.CreatedAt)
        };

        var total = await q.CountAsync(ct);
        var items = await AssessmentProjection.ProjectSessionList(_db, q)
